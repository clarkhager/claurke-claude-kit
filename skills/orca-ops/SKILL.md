...
- **A lane's work exists on ONE disk until you push it.** Orca never pushes a branch automatically. A worktree is a local checkout; the lane commits into it and stops there. Until someone runs `git push -u origin <branch>`, the only copy of that work is M1's SSD, and `orca worktree rm --force` destroys it. **Push at the first commit, not at merge time** - the value of a push is highest before you know whether the lane is any good. (2026-07-20: a sweep found THREE lanes single-copy at once - `jad-157-semantic-verify`, 5 commits and Codex-APPROVED, existed nowhere but M1; `jad-28-semantic-gen1` held `haiku_context_drain.py`, 12KB that `git log --all` proved had never been committed anywhere; and the P1-delta lane fired that same session was unpushed within minutes of creation.) **Sweep check:** `git -C <path> ls-remote --heads origin <branch> | wc -l` returns 0 for an unpushed branch. Run it on every lane, not just old ones.
...
**This section failed to fire on 2026-07-20 and the failure is instructive.** Cowork wrote a full Rule-18 Claude Code kickoff - paste-ready `cd <repo> && claude` block and all - handed it to Clark, and only produced a lane map after Clark asked "you are meant to be running our coding sessions through Orca yourself, did you somehow forget that?". The guidance below was already correct and already documented the identical 2026-07-13 failure. Nothing made anyone read it. **Root cause: the Wherehouse Rule 18(a) leads with the Code-session kickoff format and carries the Orca variant as a one-line pointer at the end, so the default path renders correctly and the pointer is skipped.** A convention that only works if you already remembered it is not a convention. **Tell on sight: any paste-ready `cd <repo> && claude` block handed to Clark for work in a repo Orca already manages.** If you are about to write one, you are in the wrong artifact - write the lane map below instead.
...
- **2026-07-20 (iteration 2)** - two findings, both from a single Cowork session that fired four lanes. (a) **Lanes are never auto-pushed** and nothing in this skill said so; three lanes were single-copy simultaneously, one holding a file never committed anywhere. Added as a convention under "The model", with the `ls-remote` sweep check. (b) **The Orca-lane kickoff section did not fire** - a Code-style kickoff was handed to Clark for work Cowork could fire itself, reproducing the 2026-07-13 failure this skill already documents. That is retro question 3 (a practice that failed because nothing triggered it), not a wording problem: Rule 18(a) leads with the Code format and demotes the Orca variant to a trailing pointer. Added an explicit tell-on-sight so the wrong artifact is recognisable while being written rather than after being sent.
